...
rv_q90_prior = halfnorm(loc=0.001, scale=.5)
...
prior_distribution["q90_stress"] = rv_q90_prior.rvs(size=nsample_prior)
prior_distribution["mu_stress"] = np.log(prior_distribution.q90_stress) -\
    1.64 * prior_distribution.s_stress
prior_distribution["scale_stress"] = np.exp(prior_distribution.mu_stress)
...
plt.show()
fig.savefig(f"Figures/{today}_prior_distribution_nsample{nsample_prior}_q90prior.jpg")
...
posterior_df = pd.DataFrame(posterior_list, columns=["s_stress", "scale_stress", 
                                                     ...
                                                     "likelihood"])
posterior_df.to_excel(f"{today}_posterior_distribution_nsample_prior{nsample_prior}_nsample_lik{nsample_likelihood}_nparts{nparts}_q90prior.xlsx", 
                      index=False)
...
ax[0].set_ylabel("q90_stress")
plt.show()
fig.savefig(f"Figures/{today}_posterior_distribution_nsample_prior{nsample_prior}_nsample_lik{nsample_likelihood}_nparts{nparts}_q90prior.jpg")
